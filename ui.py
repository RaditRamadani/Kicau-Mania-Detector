...
def draw_debug_overlay(frame: np.ndarray, data: Dict) -> None:
    """Draw debug info (mouth center, hand landmarks, trajectory, state) on frame."""
...
class GifPlayer:
    """Load and play GIF frames as OpenCV images."""
    ...
    def _load(self) -> None:
        try:
            ...
            print(f"[WARNING] GIF not found: {self.gif_path}")
            self.loaded = False
            return
...
class GifWindow:
    """Manages a separate OpenCV window for GIF display."""
